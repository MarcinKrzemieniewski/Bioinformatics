...
f = open('new_data/lab12/Drosophila_melanogaster.BDGP6.32.106.gtf')
for line in f:
	if line.startswith('#'):
		continue
	line = line.split()
	if line[2] == 'gene':
		...
		geneid = line[9].rstrip(';').strip('\"') + ' ; ' + line[11].rstrip(';').strip('\"')
		genes[geneid] = length
	if line[2] == 'transcript':
		...
		if geneid not in trans:
			...
			trans[geneid] += 1
	if line[2] == 'exon':
		transid = line[11].rstrip(';').strip('\"') + ' ; ' + line[21].rstrip(';').strip('\"')
